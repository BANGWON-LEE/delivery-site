...
String gottenSql="";//정렬할 sql
String what = (String)session.getAttribute("what");
if(session.getAttribute("what")==null){
	what = "1";
}
...
int cnt=0;
if(what.equals("1")){
	request.setAttribute("gottenSql", "select s_name,s_star,s_intro,s_code,s_addr,s_img,s_minOrder,s_deliverytime from store where s_addr like '%"
			+ loc + "%' and s_category=? and s_progress='Y' order by s_code asc limit ?,?") ;
%>
<c:set var="storeList" value="${storeDAO.storeList(sessionScope.loc, param.s_category, sessionScope.u_code, start, recNum, gottenSql)}" />
<%
}else if(what.equals("2")){
	request.setAttribute("gottenSql", "select s_name,s_star,s_intro,s_code,s_addr,s_img,s_minOrder,s_deliverytime from store where s_addr like '%"
			+ loc + "%' and s_category=? and s_progress='Y' order by s_star desc limit ?,?") ;
%>
<c:set var="storeList" value="${storeDAO.storeList(sessionScope.loc, param.s_category, sessionScope.u_code, start, recNum, gottenSql)}" />
<%
}else if(what.equals("3")){
	request.setAttribute("gottenSql", "select s_name,s_star,s_intro,store.s_code,s_addr, count(*),s_img,s_minOrder,s_deliverytime FROM store left outer join review on store.s_code=review.s_code where s_addr like '%"
			+loc+"%' and s_category=? and s_progress='Y' group by store.s_code order by count(*) desc limit ?,?") ;
%>
<c:set var="storeList" value="${storeDAO.storeList(sessionScope.loc, param.s_category, sessionScope.u_code, start, recNum, gottenSql)}" />
<%
}else if(what.equals("4")){
	request.setAttribute("gottenSql", "select s_name,s_star,s_intro,s_code,s_addr,s_img,s_minOrder,s_deliverytime from store where s_addr like '%"
			+ loc + "%' and s_category=? and s_progress='Y' order by s_minOrder asc limit ?,?") ;
%>
<c:set var="storeList" value="${storeDAO.storeList(sessionScope.loc, param.s_category, sessionScope.u_code, start, recNum, gottenSql)}" />
<%
}else {
	request.setAttribute("gottenSql", "select s_name,s_star,s_intro,s_code,s_addr,s_img,s_minOrder,s_deliverytime from store where s_addr like '%"
			+ loc + "%' and s_category=? and s_progress='Y' order by substring(store.s_deliverytime, 1, 2) asc limit ?,?") ;

%>
<c:set var="storeList" value="${storeDAO.storeList(sessionScope.loc, param.s_category, sessionScope.u_code, start, recNum, gottenSql)}" />
<%	
}
if(request.getParameter("s_category") == null){
%>
   <script>
      alert("잘못된 접근입니다.");
                ...
                <tr>
                    <td>
                        <div class="s_list_td">
                            ...
                            <c:if test="${not empty item.m_file}">
                           		<c:set var="img" value="${item.m_file}"/>
                            </c:if>
                            <c:if test="${empty item.m_file}">
                           		<c:set var="img" value="images/noimg.jpg"/>
                            </c:if>
                            <c:if test="${empty item.m_file}">
                            <img src="./images/noimg.jpg" alt="이미지없음" id="s_main_img">
                            </c:if>
                            <c:if test="${not empty item.m_file}">
                                <img src="./images/${item.m_file}" alt="${item.m_file}" id="s_main_img">
                            </c:if>
                                <div class="s_list_s">
									...
									<div class="store_c">최소 주문금액 :<span>${item.s_minOrder }</span></div>
									<div class="store_c">배달 시간 :<span>${item.s_deliverytime }</span></div>
									<div class="like_img">
 				...
 				<img src="./images/${item.s_addr}" id="map<%=cnt %>" class="s_list_map">지도</div>
			    ...
			    // 정상적으로 검색이 완료됐으면 
			     if (status === kakao.maps.services.Status.OK) {
			        ...
			        var infowindow = new kakao.maps.InfoWindow({
			            content: '<img src="./images/${item.s_addr}" style="width:150px;text-align:center;padding:6px 0;"/>'
			        });
...
%>
		<tr>
			...
			%>
					<a href='store_list.jsp?s_category=<%=category%>&loc=<%=loc%>&pagenum=<%=i%>'><%=i%></a>													
		<%
				}
		%>
			</td>
...
$(function(){
	$("select").on('change',function(){
		...
		location.href ="store_list_orderBy.jsp?what="+what;
	})
})
</script>